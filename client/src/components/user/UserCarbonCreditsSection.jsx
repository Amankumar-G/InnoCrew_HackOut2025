...
const UserCarbonCreditsSection = ({ theme, token }) => {
  ...
  // Fetch plantations data for the current user
  useEffect(() => {
    const fetchUserPlantations = async () => {
      try {
        setLoading(true);
        const response = await fetch(
          `${import.meta.env.VITE_URL}/api/plantation/user`,
          {
            method: "GET",
            headers: {
        ...
        if (!response.ok) {
          throw new Error("Failed to fetch user plantations");
        }
        ...
        setError("Failed to load your carbon credit data");
      ...
      >
        ...
        {plantations.length === 0 ? (
          <div className="py-12 text-center">
            <FaLeaf className="w-16 h-16 mx-auto mb-4 text-gray-400" />
            <p
              className={`text-lg ${
                theme === "dark" ? "text-gray-300" : "text-gray-600"
              }`}
            >
              No carbon credit plantations yet
            </p>
            <p
              className={`text-sm ${
            ...
            >
              Register your plantation to start earning carbon credits
            </p>
          </div>
        ) : (
          ...
          >
            ...
            <div className="space-y-4 mb-6">
              ...
              <div>
                ...
                <span
                  className={`px-2 py-1 rounded-full text-xs font-semibold border ${getMarketplaceStatusColor(
                    selectedPlantation.marketplaceStatus
                  )}`}
                >
                  {selectedPlantation.marketplaceStatus === "listed"
                    ? "Listed for Trading"
                    : "Not Listed"}
                </span>
              </div>
...
export default UserCarbonCreditsSection;